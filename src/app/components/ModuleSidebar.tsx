...
// ⬇️ removed ModuleStudentControls import
    ...
    >
      ...
      <div className="shrink-0 border-b border-slate-200 bg-white/90 backdrop-blur supports-[backdrop-filter]:backdrop-blur px-3 py-3">
        <div className="grid grid-cols-[auto,1fr,auto] items-center gap-2">
          ...
          {/* spacer / could place a global action later */}
          <div />
        </div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {pills.map(({ key, label, count }) => (
                ...
                >
                  {label}
                  {typeof count === "number" && (
                    <span className={["rounded-full px-1.5 py-0.5 text[10px]", tab === key ? "bg-white/20" : "bg-slate-100"].join(" ")}>
                      {count}
                    </span>
                  )}
                </button>
      ...
      <div className="flex-1 overflow-y-auto px-3 py-4 space-y-6 pr-2">
        ...
        {/* STUDENT CONTROLS — removed per request */}
        {/* (no ModuleStudentControls rendering) */}
      </div>
