...
function uuidLike() { return (typeof crypto !== "undefined" && crypto.randomUUID) ? crypto.randomUUID() : Math.random().toString(36).slice(2); }
...
export default function AssignmentView({
  ...
  /* Student: attempts used + grade + feedback (from assignment_submissions) */
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // ✅ Latest graded submission
        const { data: lastSub } = await supabase
          .from("assignment_submissions")
          .select("grade, feedback, submitted_at")
          .eq("assignment_id", assignmentId)
          .eq("student_id", studentId)
          .order("submitted_at", { ascending: false }) // fix: no invalid 'nulls' option
          .limit(1)
          .maybeSingle();
  ...
  /* Teacher: roster + submission flags */
  useEffect(() => {
    if (!isTeacher || !assignmentId) return;
    ...
    const run = async () => {
      ...
      try {
        ...
        const baseStudents: Student[] = (studs ?? []) as Student[];
        ...
        const withSectionNames: Student[] = baseStudents.map((s) => ({
          ...s,
          section_name: s.section_id != null ? sectionNameById[s.section_id] ?? null : null,
        }));

        setStudents(withSectionNames);

        if (withSectionNames.length) {
          const ids = withSectionNames.map((s) => s.id);
  ...
  const renderSkeleton = (rows = 3) => (
    <div className="animate-pulse">
      ...
      <div className="mt-3 h-[60vh] w-full rounded-2xl bg-slate-100" />
  ...
  const renderFileViewer = () => {
    if (!previewUrl) {
      return (
        <div className="flex h-[60vh] w-full flex-col items-center justify-center rounded-2xl border border-dashed bg-white text-sm text-slate-500">
    ...
    if (fileExtLower === "pdf") {
      return (
        <object data={previewUrl} type="application/pdf" className="h-[74vh] w-full rounded-xl border">
          <iframe title="PDF preview" src={previewUrl} className="h-[74vh] w-full rounded-xl border" />
        </object>
      );
    }
    ...
    if (["png", "jpg", "jpeg", "gif", "webp"].includes(fileExtLower)) {
      return (
        <img
          ...
          className="h-auto max-h-[74vh] w-full rounded-xl border object-contain"
        />
      );
    }
    ...
    if (["doc", "docx", "ppt", "pptx", "xls", "xlsx"].includes(fileExtLower)) {
      ...
      return <iframe title="Office preview" src={gview} className="h-[74vh] w-full rounded-xl border" allowFullScreen />;
    }
  ...
  const renderAssignmentDetails = () => {
    const score = assignment?.max_score ?? null;
        ...
        <div className="mx-auto max-w-[1400px] px-3 sm:px-6 py-4">
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0">
              <div className="inline-flex items-center gap-1 rounded-full bg-slate-100 px-2 py-0.5 text-[11px] font-medium tracking-wide text-slate-700">
              ...
              <h1 className="mt-2 text-xl sm:text-2xl font-semibold tracking-tight text-slate-900">{assignment?.name ?? "—"}</h1>
              {assignment?.instruction ? (
                <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600 whitespace-pre-line">{assignment.instruction}</p>
              ) : null}
            </div>
            <span className={`shrink-0 rounded-full px-3 py-1 text-xs ring-1 ${
              isStudent ? "bg-emerald-50 text-emerald-700 ring-emerald-200" : "bg-indigo-50 text-indigo-700 ring-indigo-200"
            }`}>{isStudent ? "Student View" : "Teacher View"}</span>
          </div>
        </div>
      </div>
          ...
          <div className="col-span-12 lg:col-span-8 xl:col-span-9">
                ...
                <form onSubmit={handleSubmit} className="space-y-3">
                  ...
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      />
                      <label htmlFor="student-assignment-file" className="inline-flex cursor-pointer items-center gap-2 rounded-xl border px-3 py-2 text-xs hover:bg-slate-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500" title="Attach a file to your submission">
              ...
              <div className="rounded-2xl ring-1 ring-slate-200 bg-white p-4 shadow-sm mb-4">
                <div className="mb-3 flex items-center justify-between gap-3">
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-sm font-semibold">AI Grammar Check</h2>
                  </div>
                  <button
                    type="button"
                    onClick={async () => {
                    ...
                    className="inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-3 py-2 text-xs font-medium text-white transition hover:bg-emerald-700 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 disabled:opacity-50"
            ...
            <div className="mb-2 flex items-center justify-between">
              <div className="inline-flex items-center gap-2 text-sm font-semibold text-slate-800">
                <DocumentTextIcon className="h-5 w-5" aria-hidden="true" /> Preview
              </div>
              {displayName ? <span className="truncate text-xs text-slate-500" title={displayName}>{displayName}</span> : null}
            </div>
            ...
            <div className="col-span-12 lg:col-span-4 xl:col-span-3">
              ...
              <div className="rounded-2xl ring-1 ring-slate-200 bg-white overflow-hidden">
                <div className="px-4 py-3 border-b border-slate-200 bg-slate-50/60">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <UsersIcon className="h-5 w-5 text-slate-600" aria-hidden="true" />
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[11px] text-slate-500">of {totalCount}</span>
                    </div>
                  </div>
                  <p className="mt-1 text-xs text-slate-500">Your students and whether they’ve submitted this assignment.</p>
                </div>
