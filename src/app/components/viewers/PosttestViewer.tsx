...
export default function PosttestViewer({ moduleId }: { moduleId: string }) {
  ...
  const title = useMemo(() => quiz?.title || "Post-Test", [quiz]);

  if (loading) {
    return <div className="rounded-2xl border bg-white p-4">Loading Post-Test…</div>;
  }
    ...
    return (
      <div className="rounded-2xl border bg-white p-6">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg font-semibold">Post-Test</h2>
        </div>
        <p className="mt-3 text-sm text-slate-700">
          No Post-Test has been published for this module yet.
        </p>
      </div>
    );
  }
